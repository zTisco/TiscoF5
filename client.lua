...
local function UpdatePlayerInformations()
    ...
    PlayerInformations["PrimaryJob"] = ESX.PlayerData.job.label
    PlayerInformations["PrimaryJobGrade"] = ESX.PlayerData.job.grade_label
    PlayerInformations["SecondaryJob"] = ESX.PlayerData.job2.label
    PlayerInformations["SecondaryJobGrade"] = ESX.PlayerData.job2.grade_label

    PlayerInformations["Cash"] = valueCash
    PlayerInformations["BlackMoney"] = black_money
end

local function getInventory()
    ESX.TriggerServerCallback('Tisco:getInventory', function(object)
        PlayerInformations["Inventory"] = object
    end)
end
...
local function OpenMenu()
    ...
    while main_menu do
        ...
        RageUI.IsVisible(wallet_menu, function()
            ...
            if PlayerInformations["PrimaryJob"] ~= "" then
                RageUI.Separator(string.format("%s%s~s~ : %s", Config.Menu.Color.Premiere, PlayerInformations["PrimaryJob"], PlayerInformations["PrimaryJobGrade"]))
            end
            if PlayerInformations["SecondaryJob"] ~= "" then
                RageUI.Separator(string.format("%s%s~s~ : %s", Config.Menu.Color.Deuxieme, PlayerInformations["SecondaryJob"], PlayerInformations["SecondaryJobGrade"]))
            end

            local cash = string.format("Argent liquide : %s%s~s~ $", Config.Menu.Color.Premiere, tostring(PlayerInformations["Cash"]))
            ...
            local black_money = string.format("Argent sale : %s%s~s~ $", Config.Menu.Color.Deuxieme, tostring(PlayerInformations["BlackMoney"]))
        ...
        if (not RageUI.Visible(main_menu) and not RageUI.Visible(inv_menu) and not RageUI.Visible(weapon_menu) and not RageUI.Visible(wallet_menu) and not RageUI.Visible(clothes_menu) and not RageUI.Visible(vehicle_menu) and not RageUI.Visible(help_menu)) then
            main_menu = RMenu:DeleteType("main_menu")
        end
